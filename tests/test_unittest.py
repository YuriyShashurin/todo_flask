...
#Эта функциональность тоже реализована в unittest с помощью декораторов skipIf и skipUnless, которые пропускают тест, если условие в параметре декоратора выполняется или не выполняется соответственно:
...
#Если мы ожидаем, что тест упадёт, мы можем его пометить декоратором, который скажет фреймворку, что тест падает и это ожидаемое на данный момент поведение:
